...
const FilterModal: React.FC<FilterModalProps> = ({ visible, onClose }) => {
    const { routeLengthFilter, setRouteLengthFilter } = useSettings();
    const [selectedFilter, setSelectedFilter] = useState<FilterOption | null>(routeLengthFilter);

    useEffect(() => {
        setSelectedFilter(routeLengthFilter);
    }, [routeLengthFilter]);
    ...
    const applyFilter = () => {
        setRouteLengthFilter(selectedFilter);
        onClose();
    };

    const clearFilter = () => {
        setSelectedFilter(null);
        setRouteLengthFilter(null);
        onClose();
    };
